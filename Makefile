...
build-images: ## Build the images
	docker buildx build --platform linux/arm64 --tag "ghcr.io/k8sdeploy/authorizer:`git rev-parse HEAD`" --build-arg build=`git rev-parse HEAD` --build-arg version=`git describe --tags --dirty` --file ./k8s/Dockerfile .
	docker tag "ghcr.io/k8sdeploy/authorizer:`git rev-parse HEAD`" "ghcr.io/k8sdeploy/authorizer:latest"
	docker scan "ghcr.io/k8sdeploy/authorizer:`git rev-parse HEAD`"
...
.PHONY: wipeData
wipeData: # Wipe Data
	cat ./docker/drop.sql | docker exec -i authorizer_database_1 psql -U database_username -d bugfixes

.PHONY: injectData
injectData: # Wipe Data
	cat ./docker/create.sql | docker exec -i authorizer_database_1 psql -U database_username -d bugfixes
	cat ./docker/local.sql | docker exec -i authorizer_database_1 psql -U database_username -d bugfixes
...
bucket-upload: build-aws ## Put the build in the bucket
	aws s3 cp bin/authorizer-local.zip s3://tygon/tygon-local.zip --endpoint https://localhost.localstack.cloud:4566

build-aws: ## Build for AWS
	GOOS=linux GOARCH=amd64 go build -o bin/authorizer ./cmd
	zip bin/authorizer-local.zip bin/authorizer
